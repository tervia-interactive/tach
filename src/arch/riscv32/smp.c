...
int arch_smp_init(void) {
    ...
    for (uintptr_t hart = 0; hart < 32 && g_hart_count < MAX_CPUS; hart++) {
...
int arch_smp_boot_secondary(uint32_t cpu, void* entry) {
    ...
    return (int)sbi_ecall(SBI_EXT_ID_HSM, SBI_FID_HSM_HART_START,
        (long)g_harts[cpu], (long)(uintptr_t)riscv_secondary,
        (long)cpu, 0, 0, 0).error;
}
...
void arch_smp_send_ipi(uint32_t cpu, uint32_t vector) {
    ...
    uintptr_t base = g_harts[cpu] & ~(uintptr_t)31u;
